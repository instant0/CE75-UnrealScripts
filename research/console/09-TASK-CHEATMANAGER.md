...
- 2026-08-01 — Follow-up review: corrected the UE4/UE5 spawn-entry-point claim (`AddCheats` vs `SpawnCheatManager`, incl. the UE4 `AllowCheats` gate) and re-added the online source references the original research had omitted.
- 2026-08-01 — Implemented `UEngine_setupCheatManager()` in `console.lua` (after `UEngine_createConsole`, §Task 9). Option C exactly: reuse `state.playerController`, one property walk for `{'CheatManager','CheatClass'}`, patch `CheatClass` first, gate the spawn on `cheatCDO` (`state.blocked.cheat` on failure), replicate `NewObject<UCheatManager>(this, CheatClass)` via `UEngine.SCOAddr` with the corrected `templateOff=0x28` params fill (`Class=CheatClass`, `Outer=pc`, `SetFlags=0`, `Template=nil`), validate `Class==CheatClass` before the write, then `writePointer(pc+cmProp.offset,newCM)`; idempotent (`true,'already enabled'`). Syntax gate passed (`luac -p` + `loadfile`); mock test still 28/28. Also applied the §7d `templateOff` fix to `UEngine_createConsole` itself (was `align8(internalOff+4)`, now `align8(internalOff+6)` = 0x28 for both FName sizes).
