...
| 9 | `UEngine_setupCheatManager()` — reuse `UEngine.DevConsoleState.playerController`, patch `CheatClass`, spawn `CheatManager` via Task 7 `UEngine.SCOAddr` replication (NewObject, Option C) **gated on `UEngine.DevConsoleState.cheatCDO`**; verify by re-read. | **None** |
