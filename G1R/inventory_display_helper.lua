...
--- Ensure GNames (0 AOB if RVA/cache works)
function InventoryDisplay_EnsureGNames()
    local g = rawget(_G, "GNamesBase")
    if validateGNames(g) then return g end
    local base = getAddress(GAME)
    if base then
        local cand = base + GNAMES_RVA
        if validateGNames(cand) then
            _G.GNamesBase = cand
            _G.GNamesRVA = GNAMES_RVA
            pcall(function() registerSymbol("GNames", cand, true) end)
            print(string.format("[InventoryDisplay] GNames=0x%X (exe+0x%X)", cand, GNAMES_RVA))
            return cand
        end
    end
    print("[InventoryDisplay] GNames not found (set GNamesBase or update GNAMES_RVA)")
    return nil
end
